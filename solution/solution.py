...
def run_assistant(
    ...
    while max_turns is None or num_turns < max_turns:
        ...
        history = history[-6:]
        usage = estimate_cost(user_message, reply, OPENAI_MODEL)
...
if __name__ == "__main__":
    ...
    if not api_key or api_key in ("sk-your-key-here", "nvapi-your-key-here"):
        print("=" * 65)
        print("⚠️  CHƯA CẤU HÌNH API KEY THẬT TRONG FILE .env")
        print("=" * 65)
        print("File .env đã được tạo tự động cho bạn, nhưng đang chứa key mẫu.")
        print("Để chạy demo gọi API thật:")
        print("  1. Mở file .env")
        print("  2. Thay 'sk-your-key-here' bằng API key thật (OpenAI hoặc NVIDIA NIM).")
        print("\n💡 Lưu ý: Bài làm Block 1 đã hoàn thành và đã pass toàn bộ test!")
        print("Để kiểm tra chấm điểm (dùng mock, KHÔNG cần API key), hãy chạy:")
        print("  pytest tests/test_part1.py -v")
        print("=" * 65)
    else:
        try:
            print("=== So sánh model ===")
            ...
            print(f"\n❌ Lỗi khi gọi API: {err}")
            print("Vui lòng kiểm tra lại tính hợp lệ của API key hoặc đường truyền mạng.")
